...
mdoc_root_post(MDOC_ARGS)
{
	...
	PAIR_CLASS_INIT(&tag, "man-decor man-foot");
	t = print_otag(h, TAG_OL, 1, &tag);

	PAIR_CLASS_INIT(&tag, "tl");
	print_otag(h, TAG_LI, 1, &tag);
	print_text(h, m->os);
	print_stagq(h, t);

	PAIR_CLASS_INIT(&tag, "tc");
	print_otag(h, TAG_LI, 1, &tag);
	print_text(h, m->date);
	print_stagq(h, t);

	snprintf(title, BUFSIZ - 1, "%s(%s)", m->title, m->msec);
	PAIR_CLASS_INIT(&tag, "tr");
	print_otag(h, TAG_LI, 1, &tag);
	print_text(h, title);
	print_tagq(h, t);
...
mdoc_root_pre(MDOC_ARGS)
{
	...
	PAIR_CLASS_INIT(&tag, "man-decor man-head");
	t = print_otag(h, TAG_OL, 1, &tag);
...
mdoc_bl_pre(MDOC_ARGS)
{
	...
	PAIR_INIT(&tag[0], ATTR_CLASS, buf);
	...
	case(LIST_hyphen):
		/* FALLTHROUGH */
	case(LIST_item):
		print_otag(h, TAG_UL, 1, tag);
		break;
	case(LIST_enum):
		print_otag(h, TAG_OL, 1, tag);
		break;
	case(LIST_diag):
		/* FALLTHROUGH */
	case(LIST_hang):
...
mdoc_d1_pre(MDOC_ARGS)
{
	...
	if (MDOC_Dl == n->tok) {
		print_otag(h, TAG_PRE, 0, NULL);
	} else {
		print_otag(h, TAG_BLOCKQUOTE, 0, NULL);
	}
